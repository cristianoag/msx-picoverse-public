...
REV, DATE = '1.4', '2026-09-19'   # U2 F면 이설 반영
...
NOTE = {
 ...
 'U2': 'UDA1334A I2S DAC 모듈 (Adafruit 호환) / ★ 기판 앞면(F면) 실장 (rev1.4에서 뒷면→앞면 이설) / '
...
ASSY = [
 ...
 ('U2', 'rev1.4에서 기판 앞면(F면)으로 이설되었습니다. 이 보드에 뒷면 부품은 없습니다'),
...
CHANGES = [
 ...
 ('U2 (UDA1334MOD)', '기판 뒷면(B.Cu) → 앞면(F.Cu) 이설. 하판 셸 간섭이 해소되고, 3.5mm 잭이 기판 좌측 외곽선 밖으로 3.13mm 돌출하도록 180도 회전 배치'),
]
