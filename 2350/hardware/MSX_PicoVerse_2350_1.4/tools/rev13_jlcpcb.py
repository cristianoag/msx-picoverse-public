...
제외 대상
  · DNP 부품            (J3, R43, R44, C16)
  · 기판 일체 형상       (J11 엣지핑거, PAD01/PAD02 마운팅홀)

  python3 tools/rev13_jlcpcb.py
"""
import os, re, io, sys, math, collections

HERE = os.path.dirname(os.path.abspath(__file__))
PROJ = os.path.abspath(os.path.join(HERE, '..'))
PCB = os.path.join(PROJ, 'MSX_PicoVerse_2350_1.3.kicad_pcb')
OUT_BOM = os.path.join(PROJ, 'MSX_PicoVerse_2350_1.3_JLCPCB_BOM.csv')
OUT_CPL = os.path.join(PROJ, 'MSX_PicoVerse_2350_1.3_JLCPCB_CPL.csv')

SKIP_REF = {'J11', 'PAD01', 'PAD02'}          # 기판 일체
pcb = io.open(PCB, encoding='utf-8').read()

# ---------- 값 정규화 / 패키지 표기 (한글 BOM 과 동일 규칙) ----------
def norm(v):
    v = v.strip()
...
PKG = {'C0603': '0603', 'C0805': '0805', 'C1206': '1206', 'C3225': '1210',
       ...
       'SK-12DO2': 'SK-12D02', 'Conn_uSDcard': 'uSD-Socket',
...
while True:
    ...
    if i < 0: break
    j = pcb.find('\n\t(footprint', i + 1)
    blk = pcb[i: j if j > 0 else len(pcb)]
    ...
    parts.append({
        'ref': ref, 'val': norm(val.group(1) if val else ''), 'fp': fp,
        'x': float(at.group(1)), 'y': float(at.group(2)),
        'rot': float(at.group(3) or 0) % 360.0,
        'side': 'bottom' if lay.startswith('B.') else 'top',
        'dnp': 'dnp' in attr,
        # attr 는 신뢰할 수 없다(누락되거나 smd 로만 표기된 풋프린트가 있음).
        # 실제 패드 타입으로 판정한다. 도금 스루홀 패드가 하나라도 있으면 THT/혼합.
        'tht': bool(re.search(r'\(pad "[^"]*" thru_hole ', blk)),
    })
    i = j if j > 0 else len(pcb)
...
# ---------- CPL ----------
def q(s): return '"%s"' % str(s).replace('"', '""')
...
with io.open(OUT_BOM, 'w', encoding='utf-8', newline='') as f:
    ...
    for (val, pk), refs in order:
        ...
        f.write('%s,%s,%s,\n' % (q(val), q(','.join(refs)), q(pk)))

# ---------- 리포트 ----------
print('=' * 74)
print('JLCPCB SMT 발주 파일 생성')
print('=' * 74)
...
xs = [p['x'] for p in place]; ys = [-p['y'] for p in place]
print('  배치 범위  X[%.3f .. %.3f]   Y[%.3f .. %.3f]' % (min(xs), max(xs), min(ys), max(ys)))
...
print('\n[확인 필요] 스루홀 %d개 (JLCPCB SMT 로는 실장 불가 - 주문 화면에서 해제하거나'
      ' THT 조립 옵션 선택) :' % len(tht))
for p in tht:
    print('     %-6s %-16s %s' % (p['ref'], p['val'], PKG.get(p['fp'], p['fp'])))
...
print('   ' + '  '.join('%s도:%d개' % (k, v) for k, v in sorted(rc.items(), key=lambda kv: float(kv[0]))))
