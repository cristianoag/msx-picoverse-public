...
- Kept the stub byte budget unchanged at 14 bytes for a single padding run and 8 + 9 bytes for the two-cave split, so placement, header safety and the 120-ROM placement results are exactly as in v2.46.
...
- Verified on the Panasonic FS-A1FX and Philips NMS 8245 openMSX profiles with their real BIOS dumps, driving the ROMs through the actual firmware patch code: on the FS-A1FX (60 Hz default) forcing 50 Hz gives R9/RG9SAV `02`/`82`, and on the NMS 8245 (50 Hz default) forcing 60 Hz gives `00`/`80`, with King's Valley II reaching its title screen in every case and header offsets `#4004`..`#400F` left at zero.
